...
Il pannello di ricerca presente nella parte alta della colonna
...
Per quel che riguarda invece la gestione delle singole cartelle una cosa
di fondamentale importanza da considerare è che, ovviamente, ogni
singolo documento dovrà per forza di cose essere contenuto all'interno
di una specifica cartella.

In conseguenza di ciò è evidente che al livello più alto della struttura
(**Documenti**) sarà possibile solamente creare delle nuove cartelle
all'interno delle quali andranno poi inseriti i vari documenti piuttosto
che eventualmente altre sottocartelle.
...
> (pulsante "**Scarica Documento**") o la visualizzarne dell'anteprima
> (pulsante "**Anteprima**")
...
> Non avrà invece la possibilità di variare le proprietà delle cartelle
> già presenti all'interno della cartella in esame (e quindi non create
> da lui) ne, allo stesso modo, potrà eliminare queste cartelle o i
...
ereditato anche da tutte le eventuali 'Sottocartelle'
...
'Padre', e anche quest'ultimo verrà notificato in merito all'inserimento
di un nuovo documento.

**NOTA BENE:** non è possibile selezionare dei gruppi come
"Amministratore di Cartella".

**Da sottolineare infine, la possibilità di creare in blocco un'intera
struttura di cartelle e sottocartelle (con eventuali file già presenti
in esse) partendo da un archivio compresso (file .zip) precedentemente
creato.**

Per far questo è sufficiente cliccare sul pulsante **Scegli File** **(**
![](./assets/media/image95.png) **)** presente, nella maschera di
creazione Nuova Cartella, a fianco dell'etichetta **"Zip con struttura
Cartelle"**

In questo modo sarà infatti possibile uplodare il file .zip desiderato
...
**Nuova Cartella** (
![](./assets/media/image85.png) ): consente di creare, attraverso la
...
**Elimina (Cartella)** (
![](./assets/media/image97.png) ): consente di eliminare la cartella attualmente
selezionata.
...
**Proprietà** (
![](./assets/media/image99.png) ): consente di variare le proprietà
...
**Sposta Cartella** (
![](./assets/media/image100.png) ): consente di attivare la modalità di
