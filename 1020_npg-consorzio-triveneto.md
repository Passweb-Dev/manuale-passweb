# NPG CONSORZIO TRIVENETO



La modalità di pagamento etichettata a default come **"NPG Consorzio
Triveneto"** viene riconosciuta e gestita da tutte le banche che
utilizzano il relativo gateway di pagamento.
...
1.  Codificare all'interno della Tabella Pagamenti del gestionale uno
    ...
    line in oggetto assegnandogli, ad esempio, la descrizione CONSORZIO
    TRIVENTO
...
- **ID TranPortal:** codice identificativo assegnato all\'esercente;

- **Password TranPortal:** password utente;
...
**NOTA BENE**: Consorzio Triveneto gestisce solamente importi in euro;
per eventuali ordini in valuta il totale del documento verrà quindi
opportunamente convertito in euro e passato alla banca

**NOTA BENE**: quanto indicato in questo manuale relativamente
all'utilizzo dell'applicazione di Back Office del sistema "Consorzio
Triveneto" potrebbe non essere allineato con la corrente versione di
tale software. Si consiglia quindi di utilizzare la specifica
manualistica.
...
quelle che consentiranno di accedere al gateway di pagamento "Consorzio
Triveneto" e di poter così portare a termine il pagamento tramite carta
di credito.
...
reindirizzato alla specifica pagina di accesso del gateway "Consorzio
Triveneto":
